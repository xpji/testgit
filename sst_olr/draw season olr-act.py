...
cb=ax0.contourf(lon,lat_range,olr_season[0]-djf,levels=np.arange(-60,21),cmap=plt.get_cmap('bwr'),    \
...
ax0.set_title('DJF热带海域OLR-ACT水平分布图')
...
cb1=ax1.contourf(lon,lat_range,olr_season[1]-jja,levels=np.arange(-60,21),cmap='bwr',      \
            transform=ccrs.PlateCarree()) 
...
ax1.set_title('JJA热带海域OLR-ACT水平分布图')
ax1.set_xlabel('经度($°$)',fontsize=15)
ax1.set_ylabel('纬度($°$)',fontsize=15)
cbar1 = plt.colorbar(cb1,shrink=0.6,ticks=[-60,-40,-20,0,20],aspect=5,pad=0.01)
...
cb2=ax2.contourf(lon,lat_range,olr_season[2]-mam,levels=np.arange(-60,21),cmap='bwr',      \
            transform=ccrs.PlateCarree()) 
...
ax2.set_title('MAM热带海域OLR-ACT水平分布图')
ax2.set_xlabel('经度($°$)',fontsize=15)
ax2.set_ylabel('纬度($°$)',fontsize=15)
cbar2 = plt.colorbar(cb2,shrink=0.6,ticks=[-60,-40,-20,0,20],aspect=5,pad=0.01)
...
cb3=ax3.contourf(lon,lat_range,olr_season[3]-son,levels=np.arange(-60,21),cmap='bwr',      \
            transform=ccrs.PlateCarree()) 
...
ax3.set_title('SON热带海域OLR-ACT水平分布图')
ax3.set_xlabel('经度($°$)',fontsize=15)
ax3.set_ylabel('纬度($°$)',fontsize=15)
# contour = plt.contour(lon,lat_range, son,5,colors='grey',zorder=1,\
#                                   transform=ccrs.PlateCarree())
cbar3 = plt.colorbar(cb3,shrink=0.6,ticks=[-60,-40,-20,0,20],aspect=5,pad=0.01)


# fig.savefig('D:\\desktopppp\\sst_olr\\picture\\'+'不同季节态热带海域OLR-ACT水平分布图.tiff',format='tiff',dpi=150)
