...
# a=np.arange(-25,26)#左闭右开

for i in hadv:
     indx=(-125<=hadv)&(hadv<=-75)
...
# diff = [ olr_mean[i]-olr_mean[i-1] for i in range(len(olr_mean)-1) ]
...
ax.set_title('-100 MSE平流强度下OLR对SST敏感度关系',fontsize=20)
...
fig.savefig('D:\\desktopppp\\sst_olr\\picture\\'+'-100mse.tiff',format='tiff',dpi=150)

d={'-100mse':olr_mean}
df=pd.DataFrame(d)  
df.to_csv('D:\\desktopppp\\sst_olr\\'+'-100mse.csv') 
d1={'-100diff':diff}
df1=pd.DataFrame(d1)  
df1.to_csv('D:\\desktopppp\\sst_olr\\'+'-100diff.csv') 